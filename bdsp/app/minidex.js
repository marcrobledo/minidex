...
function renderEncounters(encounters){
	...
	for(var i=0; i<encounters.length; i++){
		...
		td2.appendChild(parseEncounterMethod(encounters[i][2],encounters[i][0]));
...
function renderLocationEncounters(encounters, groupName, showAsGrid){
	...
	for(var i=0; i<encounters.length; i++){
		if(encounters[i].group){
			...
			if(showAsGrid){
				var currentMethod=parseEncounterMethod(encounters[i][2], encounters[i][0]).innerHTML;
...
function getMachine(moveIndex, field){
	var machines=[];
	if(GENERATION>=8 && field==='m' && TRs.indexOf(moveIndex)>=0){ //TRs
		return '<small>'+localize('tr')+'</small>'+padMachine(TRs.indexOf(moveIndex));
	}else if(typeof HMs_REMAKES !== 'undefined' && HMs.indexOf(moveIndex)!==HMs_REMAKES.indexOf(moveIndex)){ //if remake HMs are different from normal games HMs (Gen IV and Gen VI only)
		if(GENERATION===6 && moveIndex===249) //special case: Rock Smash in GEN VI is TM94 in XY and HM06 in ORAS
			return '<small>'+localize('tm')+'</small>94'+'<sup>'+LEARNSET_GAMES_ID.toUpperCase()+'</sup>'+
				'<br/><small>'+localize('hm')+'</small>06'+'<sup>'+LEARNSET_REMAKES_ID.toUpperCase()+'</sup>';
		else if(HMs.indexOf(moveIndex)!==-1)
			return '<small>'+localize('hm')+'</small>'+padMachine(HMs.indexOf(moveIndex))+'<sup>'+LEARNSET_GAMES_ID.toUpperCase()+'</sup>';
		else
			return '<small>'+localize('hm')+'</small>'+padMachine(HMs_REMAKES.indexOf(moveIndex))+'<sup>'+LEARNSET_REMAKES_ID.toUpperCase()+'</sup>';
	}else if(typeof HMs !== 'undefined'  && HMs.indexOf(moveIndex)>=0){
		return '<small>'+localize('hm')+'</small>'+padMachine(HMs.indexOf(moveIndex));
	}else if(field==='mr' && TMs_REMAKES && TMs_REMAKES.indexOf(moveIndex)>=0){
		return '<small>'+localize('tm')+'</small>'+padMachine(TMs_REMAKES.indexOf(moveIndex));
	}else{
	...
	if(typeof learnset[field] !== 'undefined'){
		...
		for(var i=0; i<moves.length; i++){
			...
			if(type==='levelup'){
				...
				if(type==='machine'){
					td0.innerHTML=getMachine(moves[i], field);
...
function refreshDexNav(){	
	...
	if(NATIONAL_DEX)
		for(var i=0; i<GENERATION && MAX_NATIONAL>MAX_NATIONAL_BY_GENERATION[i]; i++){
			nDexes++;
		}
	var selectedDex;
	if(nationalMode)
